...
Before creating or refactoring components in this workspace, read `AGENTS.md`, `style-guide/style-guide.md`, `style-guide/style-guide.ts.md`, `style-guide/style-guide.html.md`, and `style-guide/style-guide.scss.md`. Those project rules override generic examples in this skill.

Apply the project defaults: `ChangeDetectionStrategy.OnPush`, signal inputs/outputs/queries, `computed()` for derived state, `inject()` over constructor DI, `host` metadata instead of host decorators, `private`/`protected readonly` members by default, explicit public/protected API types, native control flow, `class`/`style` bindings instead of `ngClass`/`ngStyle`, semantic HTML, button `type` attributes, and WCAG AA accessibility.
