...
## Project Style Guide (applies after scaffolding)

Once the app is generated, all subsequent code must follow the project style guide. Read it before generating Angular source files:

- [../../style-guide/style-guide.md](../../style-guide/style-guide.md) — index
- [../../style-guide/style-guide.ts.md](../../style-guide/style-guide.ts.md)
- [../../style-guide/style-guide.html.md](../../style-guide/style-guide.html.md)
- [../../style-guide/style-guide.scss.md](../../style-guide/style-guide.scss.md)

For the high-leverage overrides (signal-based I/O, OnPush everywhere, `inject()` over constructor DI, native control flow, `class.x` / `style.x` over `ngClass` / `ngStyle`, `AsyncPipe` + `takeUntilDestroyed`, `private`/`readonly`/`const` defaults, no `standalone: true`, etc.), see the same overrides block in [../angular-developer/SKILL.md](../angular-developer/SKILL.md).

Prefer Reactive Forms for production forms unless the user explicitly chooses experimental Signal Forms for a new Angular v21+ form or prototype.

**Scaffolding defaults for this project:**

- `--style=scss`
- `--routing`
- `--prefix=app` (or match an existing prefix in the workspace)
- `--ai-config=agents` (project uses `AGENTS.md` as the source of truth)
- Set up Husky + lint-staged + Prettier + Angular ESLint immediately after scaffolding (see [../../README.md](../../README.md)).
...
4. Remember the following guidelines for continuing to generate Angular application code:
   ...
   - To generate interfaces, use the Angular CLI `npx ng generate interface <interface-name>` only when an interface is genuinely needed; otherwise prefer `type` aliases per the workspace TypeScript style guide.
