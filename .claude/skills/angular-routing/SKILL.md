...
Before creating or refactoring routes in this workspace, read `AGENTS.md`, `style-guide/style-guide.md`, `style-guide/style-guide.ts.md`, and `style-guide/style-guide.html.md`. Those project rules override generic examples in this skill.

Apply the project defaults: lazy-load feature routes, prefer functional guards/resolvers, use `withComponentInputBinding()` and signal inputs for route parameters where suitable, keep route components focused, set `ChangeDetectionStrategy.OnPush`, use `inject()` instead of constructor DI, and keep routed templates simple and accessible.
