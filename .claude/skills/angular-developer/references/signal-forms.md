...
Signal Forms provide a reactive, type-safe, and model-driven way to manage form state using Angular Signals in Angular v21+.

**Workspace rule:** Use this reference only when the user explicitly chooses experimental Signal Forms for a new Angular v21+ form or prototype. For production forms in this repository, prefer Reactive Forms unless the existing application strategy or user request says otherwise.

When using Signal Forms, stay inside the Signal Forms API for the selected form. Do not mix `FormControl`, `FormGroup`, `FormArray`, or `FormBuilder` into the same form implementation. Do not use `null` as a field value or field type.
...
When the task explicitly selected Signal Forms, do not import `FormControl`, `FormGroup`, `FormArray`, or `FormBuilder` from `@angular/forms` for that form. Signal Forms replace these concepts within a Signal Forms implementation.
Signal Forms do not have a builder.
