...
Before creating or refactoring signal-based code in this workspace, read `AGENTS.md`, `style-guide/style-guide.md`, and `style-guide/style-guide.ts.md`. Those project rules override generic examples in this skill.

Apply the project defaults: keep writable signals private where practical, expose readonly signals or computed values, use `computed()` for derived state, use `set()` or `update()` rather than `mutate()`, prefer `readonly` members, add explicit types for public/protected APIs, group `inject()` calls at the top of classes, avoid debug logging, and use `AsyncPipe` or `takeUntilDestroyed()` for Observable interop.
