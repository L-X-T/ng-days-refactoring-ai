...
Before creating or refactoring tests in this workspace, read `AGENTS.md`, `style-guide/style-guide.md`, and `style-guide/style-guide.ts.md`. Those project rules override generic examples in this skill.

Apply the project defaults: use the workspace's configured test framework, keep tests behavior-focused, avoid implementation-coupled assertions, use strict types, avoid `any`, remove debug logging, preserve `OnPush` behavior with explicit `fixture.detectChanges()` where needed, and prefer accessible queries or harnesses for component interaction.
